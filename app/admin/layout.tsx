...
const items:MenuItemType[] = [
  ...
  useEffect(() => {
     function init(){
      ...
      const isJWT = (token:any) => {
      const jwtRegex = /^[A-Za-z0-9-_]+\.[A-Za-z0-9-_]+\.[A-Za-z0-9-_]+$/;
      return jwtRegex.test(token);
    };
      
      //verify if token is jwt
      ...
      // verify jwt token with backend (optional but recommended for security)
      // if (!token) {
      //   router.push("/");
      // }
     }
        ...
        <Header style={{ padding: 0, backgroundColor: 'lightgray', flexShrink: 0 }}>
          <div className="flex items-center justify-between px-6">
            ...
            <div className="flex items-center gap-4">
              ...
              {/* Ensure menu items are defined as an array inside an object */}
<Dropdown 
  menu={{ 
    items: [
      {
...
>
  {/* The Dropdown MUST have exactly one child element */}
  <div className="flex items-center gap-2 cursor-pointer p-1 hover:bg-gray-100 rounded-md transition-all">
    <Avatar icon={<UserOutlined />} size={28} />
